...
run_name = "nyc-DNS-dis-test10"

def dcg_score(y_true, order, k=10):
...
def ndcg_score(y_true, order, k=10):
    y_order = np.argsort(y_true)[::-1]
    best = dcg_score(y_true, y_order, k)
    actual = dcg_score(y_true, order, k)
    return actual / best


def cal_ndcg(order, label, k,vocab_poi):
    # log_prob (N, L), label (N), batch_size [*M]
    nd=0

    y_true = np.zeros(vocab_poi+1,dtype=int)
    y_true[label] = 1

    nd += ndcg_score(y_true, order, k=k)
    return nd
...
def _test(hps,batcher):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        with tf.variable_scope("Model_Train"),tf.device("/cpu:0"):
            ...
            # for step in range (198,200): #for mode 'test'
            ...
            for user in np.random.permutation(hps.nb_users):
                ...
                if (enc_poi is not None):
                    ...
                    for label_i in np.where(label[0]==1)[0]:
                        count += 1
                        ...
                        n_5 = cal_ndcg(poi_rank_candidate[0], label_i, 5,hps.nb_items)
                        n_10 = cal_ndcg(poi_rank_candidate[0], label_i, 10,hps.nb_items)
                        n_15 = cal_ndcg(poi_rank_candidate[0], label_i, 15,hps.nb_items)
                        n_20 = cal_ndcg(poi_rank_candidate[0], label_i, 20,hps.nb_items)
                        rec_5=precision(poi_rank_candidate, label_i, 5)
                        rec_10 = precision(poi_rank_candidate, label_i, 10)
                        rec_15 = precision(poi_rank_candidate, label_i, 15)
                        rec_20 = precision(poi_rank_candidate, label_i, 20)
                        # cal_ndcg(order, label, k, vocab_poi)
...
################################################
def precision(topk_poi, batch_y, k):
    precision = 0
    topk_poi=topk_poi[0][0:k]

    if batch_y in topk_poi:
        precision += 1
    return precision
#############################################

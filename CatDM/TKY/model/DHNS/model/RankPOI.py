            ...
            with tf.variable_scope("decoder"):
                ...
                for i in range(hps.batch_size):
                    ...
                    for j in range(hps.enc_timesteps-1):
                        ...
                        #예측점수 상위 50개
                        pref_val, pref_index = tf.nn.top_k(tf.gather((all_batch_stack*-1)[0],self._enc_neg_poi[i, j+1,:]), 10) #50개

                        # mm_place = self.distance[self._enc_poi[i, j+1]]
                        pref_real_index = tf.gather(self._enc_neg_poi[i, j+1,:], pref_index)

                        #
                        # val, index = tf.nn.top_k(tf.multiply(tf.gather((all_batch_stack*-1)[0],pref_real_index),tf.gather(mm_place, pref_real_index)), 1)
                        #
                        # neg_id = tf.gather(self._enc_neg_poi[i, j+1,:],index)[0]
                        u_em = tf.nn.embedding_lookup(user_embedding, self._user_id[i])
                        t_em = tf.nn.embedding_lookup(time_embedding, self._enc_time[i, j])
                        o_em = outputs[i, j]
                        neg_list = []
                        p_em = tf.nn.embedding_lookup(poi_embedding, pref_real_index)
                        ...
                        neg_c = tf.norm(tf.nn.embedding_lookup(category_embedding,
                                                               tf.nn.embedding_lookup(self._poi_category,
                                                                                      pref_real_index)) - u_em,
                                        axis=1) ** 2
                        neg_prob = neg_p + neg_s + neg_t + neg_c + tf.gather(b, pref_real_index)

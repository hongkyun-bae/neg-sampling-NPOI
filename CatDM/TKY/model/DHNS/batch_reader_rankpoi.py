...
class Batcher(object):
    ...
    def from_path_with_time(cls, path_train,path_test,path_candidate,alldata_path, hps):
        ...
        # time_windows.append(1440)
        ####################################################################
        with open("../../data/Foursquare/TKY/TKY_VENUE_CAT_LON_LAT.csv") as f1:
            for line in f1:
    ...
    def nextBatch_POI_Train(self, uid):
        ...
        if len(check_list_train) != 0:
            ...
            enc_neg_poi=np.zeros((hps.enc_timesteps,500),dtype=np.int32)

########################################################## DNS적용
            for i in range(hps.enc_timesteps):
                k=0
                for j in random.sample(set(range(0,hps.nb_items-1))-set(check_list_train[:, 0]),500):
                    enc_neg_poi[i,k]=j
                    k+=1

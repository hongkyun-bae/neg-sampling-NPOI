...
tf.flags.DEFINE_string("data_type", "Bri",
                       "the type of dataset, Bri or TKY")
...
poi = pd.read_csv('../../data/Foursquare/Bri/Bri_VENUE_CAT_LON_LAT.csv')
...
def _test(hps,batcher):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        with tf.variable_scope("Model_Train"),tf.device("/cpu:0"):
            ...
            for step in range (136,137): #for mode 'test'

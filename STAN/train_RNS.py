...
def sampling_prob(prob, label, num_neg,history):
    ...
    not_history=list(set(all_index)-set([x-1 for x in history.cpu().detach().numpy()]))#exclude history
    global global_seed
    random.seed(global_seed)
    global_seed += 1

    sample_list = random.sample(not_history, num_neg)
    val, index = torch.topk(prob[0, sample_list], k=num_neg, sorted=False)
...
if __name__ == '__main__':
    ...
    for i in range(l_max):
        nonzero[i]=place_correlation[i][0].tolist()
    # print(nonzero)

    methodname=str(part)+'RNS'

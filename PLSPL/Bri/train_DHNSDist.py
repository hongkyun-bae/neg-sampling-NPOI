...
data = pd.DataFrame(pd.read_table("input/bri_small2.txt", header=1,sep='\t', encoding="latin-1"))
...
for epoch in range(num_epochs):
    ...
    for step, (batch_x, batch_x_cat, batch_y, hours, batch_userid, hour_pre, week_pre) in enumerate(loader_train):
        ...
        poi_candidate = list(range(vocab_poi))
        ...
        for i in range(batch_x.size(0)):
            for j in range(n_ep):
                ...
                neg_ind = torch.topk(outputs[i,j,ran], 20, dim=-1).indices
                top20 = [ran[i] for i in neg_ind]
                neg_dis_ind=torch.topk(torch.mul(outputs[i,j,top20],torch.Tensor(place_correlation[batch_y[i, j].cpu().detach().numpy()-1][top20]).to('cuda:0')), 10).indices

...
import scipy.sparse
# corr= scipy.sparse.load_npz('./corr/tky_place_correlation_50.npz')
...
data = pd.DataFrame(pd.read_table("input/tky_small.txt", header=None, encoding="latin-1"))
...
checkpoint = torch.load('./checkpoint_file/checkpoint_tky-random-small3/checkpoint_51.tar')
...
def precision(indices, batch_y, k, count):
    precision = 0
...
def recall(indices, batch_y, k, count, delta_dist):
    ...
    for i in range(indices.size(0)):
        ...
        if batch_y[i].long() in sort[:k]:
            recall_correct += 1
    return recall_correct /indices.size(0)/ count
...
for step, (batch_x, batch_x_cat, batch_y, hours, batch_userid, hour_pre, week_pre) in enumerate(loader_test):
    ...
    precision_1 += precision(indices, batch_y2, 1, count)
    precision_5 += precision(indices, batch_y2, 5, count)
    precision_10 += precision(indices, batch_y2, 10, count)
    precision_15 += precision(indices, batch_y2, 15, count)
    precision_20 += precision(indices, batch_y2, 20, count)
    precision_25 += precision(indices, batch_y2, 25, count)
    precision_50 += precision(indices, batch_y2, 50, count)

...
def train(model, train_dataset, test_dataset, optimizer, loss_fn, negative_sampler, test_sampler, region_processer,
    ...
    for epoch_idx in range(num_epochs):
        ...
        model.save('./save/Bri@DNS--epoch'+str(epoch_idx)+'.pth')
        ...
        print("{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f} ".format(r5, r10, r15, r20, ndcg[4],
                                                                                ndcg[9], ndcg[14], ndcg[19]))
        print(sum([r5, r10, r15, r20, ndcg[4], ndcg[9], ndcg[14], ndcg[19]]))

        if args.results_path:
            with open(args.results_path, 'a') as f:
                print("{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f} ".format(r5, r10, r15, r20, ndcg[4],
                                                                                        ndcg[9], ndcg[14], ndcg[19]),file=f)
                print(sum([r5, r10, r15, r20, ndcg[4], ndcg[9], ndcg[14], ndcg[19]]),file=f)
...
if __name__ == "__main__":
    ...
    config = unserialize('./data/DHNSconfig.json')
    # parser.add_argument('--config', type=str, required=True)
    # parser.add_argument('--dataset', type=str, required=True)
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--eval_samples', type=str, default=False)
    parser.add_argument('--save_path', type=str, default='')
    # parser.add_argument('--load_path', type=str, default='./save/Bri@DNS--epoch78.pth')
    ...
    parser.add_argument('--results_path', type=str, default='./results/test result_Bri@'+ 'DNS' + '.txt')
    ...
    if config["train"]["negative_sampler"] in {"UniformNegativeSampler", "DNS", "KNNSampler",
                                               "KNNWRMFSampler"}:
    ...
    args.save_path = './results/Bri@DNS-' + '.pth' #여기

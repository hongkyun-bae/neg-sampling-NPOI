...
class LBSNDataset(Dataset):
    def __init__(self, filename):
        # filename: 0 user, 1 time, 2 lat, 3 lon, 4 loc
        ...
        self.loc2count = {}
        self.n_loc = 1
        # loc_index,loc,gps establish relationship between the three, location frequency
        self.build_vocab(filename)
        print(f'{self.n_loc - 1} locations')
        # [user_idx, location, time, region, boolean] sorted by time
        self.user_seq, self.user2idx, self.region2idx, self.n_user, self.n_region, self.region2loc, self.n_time = self.processing(
            filename)
    ...
    def region_stats(self):
        ...
        for i in range(len(bin_edges) - 1):
            print("#loc in [{}, {}]: {:d} regions".format(math.ceil(bin_edges[i]), math.ceil(bin_edges[i + 1] - 1),
                                                          hist[i]))
    ...
    def processing(self, filename, min_freq=100):
        user_seq = {}
        user_seq_array = list()
        region2idx = {}
        idx2region = {}

...
def collect_fn_quadkey(batch, data_source, sampler, region_processer, loc2quadkey=None, k=5, with_trg_quadkey=True):
    ...
    if with_trg_quadkey:
        ...
        # [(1+k) * T, N, LEN_QUADKEY)
        batch_trg_regs = batch_trg_regs.permute(2, 1, 0, 3).contiguous().view(-1, batch_trg_regs.size(0),
...
def collect_fn_test(batch, data_source, sampler, region_processer, loc2quadkey=None, k=100, with_trg_quadkey=True):

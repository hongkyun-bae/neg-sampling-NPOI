...
def evaluate(model, test_dataset, negative_sampler, region_processer, loc2quadkey, device, batch_size=32, num_neg=100,
    ...
    loader = DataLoader(test_dataset,
                             ...
                             collate_fn=lambda e: collect_fn_test(e, test_dataset, neg_sampler.test_(test_dataset.n_loc),
...
def save_embed(model, idx2loc, n):
    ...
    category_embedding = np.array(category_embedding.cpu())
    f = open('geosan@100_tky.txt', 'w', encoding='utf-8')
    for i, category in enumerate(idx2loc.values()):
        f.write(category + ',')
        f.write(','.join(list(map(str, category_embedding[i]))) + '\n')
...
lat = np.array([float(v[0]) for v in dataset.idx2gps.values()])#.reshape(-1, 1)
# print(lat)
lon = np.array([float(v[1]) for v in dataset.idx2gps.values()])#.reshape(-1, 1)

place_correlation = sklearn_haversine(lat, lon)
...
def train(model, train_dataset, test_dataset, optimizer, loss_fn, negative_sampler, test_sampler, region_processer,
          loc2quadkey, device, num_neg=5, batch_size=64,
    ...
    for i in range(train_dataset.n_loc):
        nonzero[i]=1-minmax_scale(place_correlation[i].tolist())
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx, (user, loc, time, region, trg, trg_reg, trg_nov, sample_probs, ds) in batch_iterator:
            ...
            pos_score, neg500_score = output.split([1, 50], -1) # 디엔에스는 500으로 하면 되고, 랜덤 는 10으로 하면됨

            neg_score,ind=torch.topk(neg500_score,20,sorted=False)


            pos_index, neg_index=trg.view(-1, loc.size(0), loc.size(1)).permute(2,1, 0).split([1, 50], -1)#[1].shape) # 32,109,20 5559,32

            top20_neg_realindex=neg_index.gather(dim=2, index=ind)
            pos_index_expanded = pos_index.expand_as(top20_neg_realindex)  # pos_index 텐서를 32x109 크기로 확장

            #----------------------------------------------------------------
            distance=torch.ones_like(pos_index_expanded)
            for i in range(pos_index_expanded.size(0)):
                for j in range(pos_index_expanded.size(1)):
                    for z in range(pos_index_expanded.size(2)):
                        distance[i,j,z]=nonzero[pos_index_expanded[i,j,z].item()][neg_index[i,j,z].item()]


            neg_score_dis,ind =torch.topk(torch.mul(distance,neg_score),10,sorted=False)
            #----------------------------------------------------------------
            # top10index=neg_index.gather(dim=2,index=ind)

            top10score=neg_score.gather(dim=2,index=ind)


            # pos_score, neg_score = output.split([1, num_neg], -1)
            # torch.ones(len(trg_seq), 10, dtype=torch.float32)
            loss = loss_fn(pos_score, top10score, sample_probs)
        ...
        model.save('./save/Bri@DNS-distance3-epoch'+str(epoch_idx)+'.pth')
...
if __name__ == "__main__":
    ...
    config = unserialize('./data/DHNSDistconfig.json')
    # parser.add_argument('--config', type=str, required=True)
    # parser.add_argument('--dataset', type=str, required=True)
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--eval_samples', type=str, default=False)
    parser.add_argument('--save_path', type=str, default='')
    parser.add_argument('--load_path', type=str, default='')#./results/Bri@DNS-distance3.pth


    #실행방법: 에폭 완료되었으면 load-path에 경로 넣고 돌리면 됨

    parser.add_argument('--results_path', type=str, default='./results/test result_Bri@'+ 'DNSDist' + '.txt')
    ...
    loc_query_tree_path = "./data/loc_query_tree.pkl"
    ...
    if config["train"]["negative_sampler"] in {"UniformNegativeSampler", "KNNPopularitySampler", "KNNSampler",
                                               "KNNWRMFSampler"}:
    ...
    # pth -> txt
    # temp_model = torch.load('./results/tky/GeoSAN@100.pth')
    ...
    args.save_path = './results/Bri@DHNSDist' + '.pth' #여기
    ...
    # checkpoint =
    # model.load_state_dict(torch.load('./save/Bri@DNS-distance-epoch55 copy 3.pth'))

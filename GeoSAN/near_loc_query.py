...
class LocQuerySystem:
    ...
    def prefetch_knn(self, k=2000):
        self.knn = 5000
        self.knn_results = np.zeros((self.coordinates.shape[0], 5000), dtype=np.int32)
        for idx, gps in tqdm(enumerate(self.coordinates), total=len(self.coordinates), leave=True):
            trg_gps = gps.reshape(1, -1)
            _, knn_locs = self.tree.query(trg_gps, 5000 + 1)
    ...
    def get_knn(self, trg_loc, k=3000):
        if 5000 <= self.knn:
            return self.knn_results[trg_loc - 1][:5000]
...
if __name__ == "__main__":
    ...
    # filename_clean = os.path.join(args.dataset, "LSBNDataset_Bri.data")
    # save_path = os.path.join(args.dataset, "loc_query_tree.pkl")
    #
    # filename_raw = ".\\data\\dataset_TSMC2014_Bri_for_GeoSAN.txt"
    # filename_clean = ".\\data\\UbiComp2016_QuadKeyLSBNDataset_Bri.data"
    # save_path = ".\\data\\loc_query_tree_Bri.pkl"
    ...
    loc_query.prefetch_knn()
    # loc_query.prefetch_radius()

...
class testSampler(nn.Module):
    def __init__(self, nloc):
        nn.Module.__init__(self)
        self.n_loc = nloc

    def forward(self, trg_seq, k, **kwargs):
        return torch.randint(1, self.n_loc, [len(trg_seq), k]), torch.ones(len(trg_seq), 1000, dtype=torch.float32)


class UniformNegativeSampler(nn.Module):
    def __init__(self,user_visited_locs, exclude_visited=True,nloc=None):
        nn.Module.__init__(self)
        self.n_loc = nloc
    ...
    def forward(self, trg_seq, k, user, **kwargs):
        ...
        neg_samples = np.zeros((len(trg_seq), 50), dtype=np.int32)
        for i in range(len(trg_seq)):
            available_locs = set(range(1, self.n_loc )) - self.user_visited_locs[user]
            sample = np.random.choice(list(available_locs), size=50, replace=False)
            neg_samples[i] = sample
        neg_samples = torch.tensor(neg_samples, dtype=torch.long)

        return neg_samples, torch.ones(len(trg_seq), 50, dtype=torch.float32)
...
class KNNtestSampler(nn.Module):
    def __init__(self, query_sys, user_visited_locs, num_nearest=1000, exclude_visited=False):
        nn.Module.__init__(self)
...
class KNNSampler(nn.Module):
    def __init__(self, query_sys, user_visited_locs, num_nearest=2000, exclude_visited=True):
        nn.Module.__init__(self)
        self.query_sys = query_sys
        self.num_nearest = num_nearest
        self.user_visited_locs = user_visited_locs
...
class KNNPopularitySampler(nn.Module):
    def __init__(self, query_sys, loc2freq, user_visited_locs, num_nearest=500, exclude_visited=True):
        nn.Module.__init__(self)
        self.num_nearest = num_nearest

...
class STKGRec(nn.Module):
    ...
    def calc_kg_loss(self, h, r, pos_t, neg_t,poi_temporal_distance_matrix,neg_ind):
        """
        h:      (kg_batch_size)
        r:      (kg_batch_size)
        pos_t:  (kg_batch_size)
        neg_t:  (kg_batch_size) ex: [batch1, batch2...이런식으로 되어있음]
        """
        ...
        r_mul_neg_t_list = []
        for i in range(neg_t_embed.size(1)):  # 500개의 요소에 대해 반복
            neg_t = neg_t_embed[:, i, :].unsqueeze(1)  # neg_t_embed의 i번째 요소 선택
            r_mul_neg_t = torch.bmm(neg_t, W_r).squeeze(1)  # torch.bmm 수행
            r_mul_neg_t_list.append(r_mul_neg_t)

        # 결과 리스트를 하나의 텐서로 변환
        r_mul_neg_t = torch.stack(r_mul_neg_t_list, dim=1)


    ###시도 1
        # neg_t_embed.unsqueeze_(2)  # (500, kg_batch_size, 1, entity_dim)
        # W_r_expanded = W_r.unsqueeze(0).unsqueeze(0)  # (1, 1, relation_dim, entity_dim)
        # result = torch.matmul(neg_t_embed, W_r_expanded)  # (500, kg_batch_size, 1, relation_dim)
        # r_mul_neg_t = result.squeeze(2)  # (500, kg_batch_size, relation_dim)
    ####################
        neg_score_list = []
        for i in range(r_mul_neg_t.size(1)):  # 500개의 요소에 대해 반복
            r_mul_neg_t_i = r_mul_neg_t[:, i, :]
            neg_score_i = torch.sum(torch.pow(r_mul_h + r_embed - r_mul_neg_t_i, 2), dim=1)
            neg_score_list.append(neg_score_i)
        ...
        top_values_50, top_indices_50 = torch.topk(neg_score, k=20, dim=1) #50번을 하기
        # neg_t[top_indices]에 해당하는 값 찾기
        batch_top_indices=[]

        for i in range(len(neg_ind)):
            batch_top_indices_list = []
            for x in top_indices_50[i]:
                batch_top_indices_list.append(neg_ind[i][x])
            batch_top_indices.append(batch_top_indices_list)
                # neg_t=[200,1,100], top_indices=200,50 200은

        poi_temporal_distance_matrix_A=1-poi_temporal_distance_matrix
        # poi_temporal_distance_matrix와 neg_t_values의 곱 계산
        ...
        for i in range(len(neg_ind)):
            dis_score = []
            n_in=0
            for x in batch_top_indices[i]: #200,50 개 즉, 50개의 topid에 대해
                dis_score.append(torch.mul(poi_temporal_distance_matrix_A[pos_t[i]][x],top_values_50[i][n_in]).item())
                n_in+=1
            dis_score_list.append(dis_score)
        # dis_score=torch.gather(poi_temporal_distance_matrix_A, 1, batch_top_indices) *top_values

        top_values, top_indices = torch.topk(torch.tensor(dis_score_list), k=10, dim=1)

        batch_top_val=[]
        for i in range(len(neg_ind)):
            batch_top_indices_list = []
            for x in top_indices[i]:
                batch_top_indices_list.append(top_values_50[i][x])
            batch_top_val.append(batch_top_indices_list)

        # Equation (2)
        kg_loss = (-1.0) * F.logsigmoid(torch.Tensor(batch_top_val).to('cuda:0') - pos_score.to('cuda:0').unsqueeze(1))

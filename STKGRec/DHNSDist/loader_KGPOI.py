...
def generate_kg_batch(kg_dict,train_batch,n_entity,device):
    ...
    for triple in train_batch:
        ...
        #if DNS or Distance
        neg_tail = sample_neg_triples_for_h(kg_dict,n_entity, head, relation, 500)

        # if RNS
        # neg_tail = sample_neg_triples_for_h(kg_dict,n_entity, head, relation, 10)
        batch_neg_tail += [neg_tail]
    neg_ind=batch_neg_tail
    ...
    return batch_head, batch_relation, batch_pos_tail, batch_neg_tail,neg_ind

def sample_neg_triples_for_h(kg_dict,n_entities, head, relation, n_sample_neg_triples):
    # print('n_sample_neg_triples',n_sample_neg_triples) ==>1
    pos_triples = kg_dict[head]
    # print('pos_triples',pos_triples) ==> [(157,22),(174,46),... 555개 넘음]
    sample_neg_tails = []
    while True:

...
class DataFoursquare(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=48, min_gap=10, session_min=3, session_max=10,
        ...
        tmp_path = "data/"
        # self.TWITTER_PATH = tmp_path + 'TKY_small_stkg.txt'
        self.TWITTER_PATH = tmp_path + 'dataset_UbiComp2016_Bri_for_GeoSAN.txt'
    ...
    def load_trajectory_from_tweets(self):
        if self.save_name == '/nyc':
            data = pd.DataFrame(pd.read_table('../preprocessing/input/nyc_small.txt', header=None, encoding="latin-1"))
            data.to_csv(self.TWITTER_PATH, sep='\t', header=False, index=False, encoding='utf-8', line_terminator='\n')

        if self.save_name == '/tky':
            data = pd.DataFrame(pd.read_table('../preprocessing/input/tky_small.txt', header=None, encoding="latin-1"))
            data.to_csv(self.TWITTER_PATH, sep='\t', header=False, index=False, encoding='utf-8', line_terminator='\n')

        if self.save_name == '/bri':
            data = pd.DataFrame(pd.read_table('../preprocessing/input/dataset_UbiComp2016_Bri_for_GeoSAN.txt', header=None, encoding="latin-1",sep=','))
            data.to_csv(self.TWITTER_PATH, sep='\t', header=False, index=False, encoding='utf-8', line_terminator='\n')
        ...
        with open(self.TWITTER_PATH,'rb') as fid:
            for line in fid:
                ...
                if self.save_name =='/tky' or self.save_name =='/nyc': # TSMC original data
                ...
                if self.save_name =='/bri':
                    ...
                    uid, pid,  lat, lon, UTC_time = line.strip('\n').split('\t')
                    time = datetime.datetime.strptime(UTC_time,"%Y-%m-%d %H:%M:%S") #获取时间
                    tim = time.strftime("%Y-%m-%d %H:%M:%S")  # 转化时间格式
        ...
        for u in pick3: #开始为每个用户划分session
            ...
            for i, record in enumerate(info):  #record储存[loc,tim]
                ...
                if i == 0 or len(sessions) == 0: #session[user]={[loc1,tim1],[loc2,tim2]}
                    ...
                    if (tid - last_tid) / 3600 > self.hour_gap or len(sessions[sid - 1]) > self.session_max: #大于24h或者每一个session的长度大于10
                        sessions[sid] = [record]
    ...
    def build_users_locations_dict(self):   #loc->loc_id
        for u in self.user_filter3: #获取用户
            ...
            if u not in self.uid_list:
                self.uid_list[u] = [len(self.uid_list), len(sessions)] #存储user字典，[user:[len(user_id),len(user_session)]]
